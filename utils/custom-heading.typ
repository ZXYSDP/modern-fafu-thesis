...
#let active-heading(level: 1, prev: true) = {
  ...
  let prev-headings = query(heading.where(level: level).before(here()))
  // 当前页面的标题
  // let cur-headings = query(selector(heading.where(level: level)).after(here()), here())
  //   .filter(it => it.location().page() == here().page())
  let cur-headings = query(heading.where(level: level).after(here())).filter(it => it.location().page() == here().page())
